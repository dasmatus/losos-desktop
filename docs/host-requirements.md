...
| **cmake**, 3.31 or newer | Eight recipes configure with it. The floor is not about features this tree uses, it is about `CMAKE_POLICY_VERSION_MINIMUM`, which arrived in 3.31 and which libqrencode and libcbor both need: their `cmake_minimum_required` asks for less than 3.5, and **CMake 4 removed pre-3.5 compatibility outright**, turning what had been a deprecation warning into `Compatibility with CMake < 3.5 has been removed from CMake`. Arch ships CMake 4; Debian trixie shipped 3.31 and accepted the old projects, which is why this surfaced only when the base changed. Every other cmake package here is at or above 3.5, most via the `<min>...<max>` range form. | fails at `losos-10-base`, member 21 |
