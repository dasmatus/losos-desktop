...
| **just**, 1.24 or newer | The repository's entrypoint is the Justfile; the legacy wrapper only forwards to it for compatibility. The version floor is `canonicalize()`, which the Justfile uses to resolve its own directory: the container recipes hand that path back to a `just` running *inside* the container, where the repository is bind-mounted at its resolved path, and a checkout reached through a symlink (`/home` on any ostree host) otherwise names a directory the container does not have. | `just check` |
| **clang, llvm-ar/nm/objcopy/strip** | The whole tree is compiled with them; `manifest/toolchain.yaml` names the exact binaries, unversioned, so a distribution that installs them under versioned names needs links made. | `tools/pm-toolchain report` |
| **clang's compiler runtime** (Arch `compiler-rt`, Debian `libclang-rt-N-dev`) | Not the one the tree builds for the musl target — the host's own. Without it clang errors out on `-fsanitize=cfi` with a missing ignorelist, so the toolchain report cannot tell whether CFI works from a flag that never compiled. | `tools/pm-toolchain report` |
| **mold** | The linker `manifest/toolchain.yaml` names, reached as `-fuse-ld=mold`, so clang must find `ld.mold` on the `PATH` pm gives a step. | `tools/pm-toolchain report` links its probe with the real `LDFLAGS` |
...
| **python3 `basedpyright`** | `just check` runs `python3 -m basedpyright` over the tracked Python helpers and gates, so the module has to be importable on the host. This is a gate tool rather than something a recipe runs inside pm's jail, so the requirement is the Python module itself rather than a distribution package name: on Arch it comes from pip or the AUR rather than the main repositories. | `just check` |
| **ninja** | Every meson build. | `just check` |
| **cmake** | Eight recipes configure with it, and both an old and a new CMake work -- but not for the same reason, so neither flag on those lines is safe to tidy away. libqrencode and libcbor declare a `cmake_minimum_required` below 3.5. **CMake 4 removed pre-3.5 compatibility outright**, turning what CMake 3.x reported as a deprecation warning into `Compatibility with CMake < 3.5 has been removed from CMake`; Arch ships CMake 4 and Debian trixie shipped 3.31, which is why this surfaced only when the base changed. `-DCMAKE_POLICY_VERSION_MINIMUM=3.5` answers it, and CMake's own documentation gives it as `versionadded:: 4.0` -- so on a CMake 3.x it is simply an unused cache variable, reported as such and otherwise ignored. Every other cmake package here is at or above 3.5, most via the `<min>...<max>` range form. | fails at `losos-10-base`, member 21 |
...
| **bzip2** | No recipe names it either, and it appears on no command line: `tar -xf` picks a decompressor from the file itself, and seven sources in `manifest/sources.lock` are `.tar.bz2`. Debian's tar is built to try `lbzip2` first and fall back to `bzip2`, so a host with neither stops at the first of them -- pcre2 -- with `tar (child): lbzip2: Cannot exec: No such file or directory`, naming a program nothing in this tree mentions. Installing `bzip2` alone is enough; `lbzip2` is only faster. | fails at `losos-10-base` |
| **diff** (`diffutils`) | Named by no recipe and on no command line either. libcap's Makefile diffs generated output against the checked-in copy to produce `capshdoc.c`, so a host without it stops with a shell inside the jail reporting `diff: command not found` -- a program nothing in this tree mentions. Easy to miss because Debian gives it away: `diffutils` is Priority: required there and is in every base image, so no list had to name it. Arch's `base` metapackage does not carry it, which is why the Containerfile names it. | fails at `losos-10-base` |
| **perl** | No recipe names it on a command line, and the base container image's `perl-base` is not enough. openssl's build system is perl end to end: `Configure` is a perl script, and the Makefile it writes runs more of them to generate the assembly and the error tables. `perl-base` is Essential and deliberately small; the modules openssl reaches for live in Debian's `perl-modules`. The recipe passes `no-docs`, which removes the pod2man pass and with it the largest of those dependencies, but not the rest. | fails at `losos-10-base` |
