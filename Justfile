...
build target="":
  ...
  build_in_container() {
    ...
    python3 "{{repo}}/tools/container" run /bin/sh -eu -c 'LOSOS_MIRROR_PORT="$1"; export LOSOS_MIRROR_PORT PM; just plugins && just build "$2"' _ "{{mirror_port}}" "$1"
  }
...
container *args:
  ...
  if [ $# -eq 0 ]; then
    ...
    python3 "{{repo}}/tools/container" run just "$@"
  fi
